...
def main() -> int:
    ...
    con.execute(f"""
        copy (
            select *, strftime(event_time, '%Y-%m-%d') as event_date
    ...
    new_rows = con.execute(f"select count(*) from read_parquet('{DST}/*/*.parquet', hive_partitioning = true)").fetchone()[0]
    assert old_rows == new_rows, f"Mất hàng! {old_rows} != {new_rows}"
    print(f"  đã nén xong thành dataset mới tại {DST}")
    return 0
